...
public class RegisterMembership implements HttpHandler {
    private final ServerConfig eventsConfig;
    private final ServerConfig reportConfig;
    private final Database database;

    public RegisterMembership(final ServerConfig eventsConfig, final ServerConfig reportConfig, final Database database) {
    ...
    public void handle(final HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            final Membership membership = new Membership(membershipId, membershipName, eventsConfig.getClock().now().getEpochSecond());
            try {
                database.addMembership(membership);
                sendDataToReportServer(membership);
                response = "Membership: id = " + membershipId + " is added";
    ...
    private void sendDataToReportServer(final Membership membership) {
        ...
        if (!result.equals("ok" + System.lineSeparator())) {
            throw new EventsException("Can't send new membership info to report server: " + result);
        }
    }
}
